...
function harness() {
  ...
  const c=vm.createContext({window:{},document:{addEventListener(){},querySelectorAll(){return [];},querySelector(){return null;},getElementById:id=>id==='stationOverlay'?overlay:list},
...
test('starting playback leaves the open station picker open',()=>{
  ...
  const c=vm.createContext({setupAudio(){},aCtx:null,hls:null,
    audio:{pause(){},load(){},canPlayType(){return false;},play(){starts++;return Promise.resolve();}},
    document:{getElementById:id=>id==='stationOverlay'?overlay:(fields[id]||={style:{}})},
    toggleMenu(){throw new Error('Playback must not dismiss the picker');}});
  vm.runInContext(section('function play(url, name)', 'function describeStationStream('),c);
