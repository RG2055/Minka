...
function boot(storage=memory()){
 ...
 const c=vm.createContext({localStorage:storage,window:{},URL,location:{href:'http://localhost:8000/',origin:'http://localhost:8000'},CSS:{supports:()=>true},document:{baseURI:'http://localhost:8000/',documentElement:{style},getElementById:id=>id==='radioWindow'?{style}:null},STORAGE:{name:'theme',accent:'accent',accentMode:'accentMode',enabled:'enabled'},FIXED_ACCENTS:{},clamp:(x,a,b)=>Math.max(a,Math.min(b,x)),parseColorToRGBStr:()=> '83,201,232',findTheme:()=>({image:'ocean.webp',chip:'#53c9e8'}),applyTheme(){},lowNode:null,vizStyle:0,setVizStyle(){}});
