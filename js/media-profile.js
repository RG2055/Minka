...
(function(){
 ...
 function localLookSettings(value={}){
  const out={};if(['classic','clean','pioneer','pixel','amp','dither'].includes(value.layout))out.layout=value.layout;
  if(typeof value.pioneerPixels==='boolean')out.pioneerPixels=value.pioneerPixels;
  if(['webamp','classic','mirror','wave'].includes(value.ampVis))out.ampVis=value.ampVis;
  if(['bayer','field','rowled','torus'].includes(value.ditherVis))out.ditherVis=value.ditherVis;
  if(['auto','on','off'].includes(value.vizFrame))out.vizFrame=value.vizFrame;
  if(value.vizPositions&&typeof value.vizPositions==='object'){out.vizPositions={};for(const layout of ['classic','clean','pioneer','pixel','amp','dither']){const point=value.vizPositions[layout];if(point&&Number.isFinite(point.x)&&Number.isFinite(point.y))out.vizPositions[layout]={x:Math.max(-1,Math.min(1,point.x)),y:Math.max(-1,Math.min(1,point.y))};}}
