...
## Spectrum rotation followup

Guest Pixel always defaults to mode 31. Other layouts randomly exhaust all 23 remaining visualizers (including Classic VU, LED, DOT VU and Buddy) before repeating; the cycle survives reloads. Pixel wave is excluded from their random pool but remains selectable manually. All modes are registered before the first random open, including the three renderers installed by the extras script. Frontend cache advances to minka-4.6.658. 384 tests pass. This frontend-only adjustment needs no further Worker deployment.
